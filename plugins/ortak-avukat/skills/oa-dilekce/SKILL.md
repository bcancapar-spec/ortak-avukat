...
**ÇIKTI ŞEMASI — 40-UYAP (v0.5.9):** yeşil makbuz kesildiğinde `teslim_paketi.py` dava kökünde muhatap-nötr dış-çıktı dizini **`40-UYAP/`** kurar — teslim ürününün kopyası (UDF + varsa aynı kök-adlı PDF/DOCX; asıl `_oa/cikti`de kalır, tek-nüsha ilkesi) ve `_damga` alanlı `teslim-makbuz-KOPYA.json` oradadır. **Avukat UYAP'a YÜKLERKEN `40-UYAP/`taki kopyayı kullanır**; `00-TESLIM.md` teslim notuna `40-UYAP/` yolu satırı yazılır. Advisory'dir: kopya hatası teslimi kırmaz, makbuz RED iken üretilmez. Doktrin: `oa-kontrol/references/cikti-semasi.md`.
