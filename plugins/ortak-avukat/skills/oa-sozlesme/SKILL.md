...
- **Scriptin işi (garantör):** kapsam eksiksizliği — zorunlu kloz kategorilerinden herhangi biri sessizce atlanmış mı, yüksek riskli kloza önlem yazılmış mı, şekil şartı ve imza yetkisi değerlendirilmiş mi. `scripts/sozlesme_denetim.py`. Script **hiçbir hukuki nitelendirme yapmaz**; yalnız alanın dolu/boş/biçimine bakar. Bugün kapattığı dört yüzey: (a) hiç doldurulmamış iskelet "temiz" damgası ALAMAZ ve en az bir `YOK-EKSIK` varken TEMİZ hükmü basılmaz; (b) `sekil_sarti` beyanı — hem `VAR` hem `YOK-GEREKSIZ` — Mevzuat MCP teyit izi (madde numarası + sorgu) taşımak zorundadır, "gereksiz" en pahalı yanlış cevaptır; (c) mod adı Türkçe-duyarlı çözülür (`TAHRIR` / `İNCELEME` / `REDLINE`), tanınmayan mod sorundur — mod seçilmeden kırmızı çizgi kapısı ölüdür; (d) `gecerlilik_katmani` bloğu (ehliyet/temsil + genel işlem koşulları) boşsa tek satır uyarı — zorunlu anahtar DEĞİLDİR, eski dosyalar düşmez.
...
2. **Kloz kloz sınıfla:** her kloz → müvekkil lehine / nötr / aleyhe + **risk bandı**. Bantlar bu parçanın kendi ekseninde tanımlıdır (sayı uydurma yasak): `kritik` · `yuksek` · `orta` · `dusuk` · `yok` — `yok`, "değerlendirildi, kloz düzeyinde taşınan risk bulunmadı" demektir, "bakılmadı" demek değildir; iskelet varsayılanı da `yok` olduğu için hiç doldurulmamış şablon bu banda değil, ayrı bir kapıya takılır. **`oa-strateji`nin güçlü/dengeli/zayıf/belirsiz bantları OLASILIK bandıdır; risk bandı değildir — iki eksen birbirine atıfla tanımlanamaz.**
...
- **oa-sure** → süreli haklar: fesih bildirim pencereleri, otomatik uzama tarihleri, zamanaşımı; kritik tarihler `_oa/dosya.md` süre flag'lerine. **İptal edilebilirlik tespit edilir edilmez `oa-sure --tur maddi` DEVREYE ALINIR** (irade sakatlığında TBK m.39, aşırı yararlanmada TBK m.28 süreleri — başlangıç anı ve son gün hafızadan yazılmaz, Mevzuat MCP'den teyitli kuralla hesaplanır); bu süre kaçırılırsa metin geçerli hâle gelir ve kloz tartışması anlamsızlaşır.
...
Usulün esasa takaddümü ailenin anayasal düsturudur. Sözleşmede bunun karşılığı **geçerlilik katmanının içerikten önce gelmesidir**: şekil şartı, imza yetkisi/temsil, ehliyet ve emredici hukuk denetimi, kloz içeriği tartışmasından ÖNCE yapılır — şekli sakat sözleşme, en parlak klozu dahi taşıyamaz. Bu katmanın iki ekseni (ehliyet/temsil ve genel işlem koşulu denetimi) kloz kategorisi olarak yazılamaz — "ehliyet klozu" diye bir kloz yoktur; bu yüzden `sozlesme.json`'da ayrı `gecerlilik_katmani` bloğunda kayda geçer ve boş kalırsa script tek satır uyarır. **İmzalanmış metinde geçerlilik tartışması yalnız hükümsüzlükle bitmez: iptal edilebilirlik (irade sakatlıkları / aşırı yararlanma) SÜREYE bağlıdır ve süre geçerse kloz tartışması anlamsızlaşır — merdivenin bu basamağı `references/kloz-cetveli.md`'de, süre hesabı `oa-sure`dedir.** Uyuşmazlık klozları (yetki/tahkim/delil sözleşmesi/tebligat) yarının usul savaşının bugünden yazılmasıdır; bu klozlar müvekkil lehine kurgulanır ve karşı taslakta İLK denetlenenler arasındadır.
