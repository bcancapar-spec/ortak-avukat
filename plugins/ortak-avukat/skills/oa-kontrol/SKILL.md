...
description: >-
  ...
  çıktısını teslimden önce süzer. Deterministik `oa-antitez` motoru bu listelerin
  üstünde çalışır.
---
...
- `python scripts/kunye_teyit.py <taslak.md>` → **A listesinin atıf-izi kapısı:** taslaktaki her içtihat/mevzuat künyesini teyit edici kaynak evreniyle çaprazlar. **Kaynak evreni SADECE ikisidir:** `_oa/teyit/kunye-teyit.md` kütüğü + `_oa/teyit/dokum/` ham MCP dökümleri (bu ikinci dizin `oa-ictihat`'ın her MCP sonucunu diske yazma adımıyla beslenir). **`_oa/cikti/` teyit kaynağı DEĞİLDİR** — orası taslak/antitez/kıyas gibi MODELİN çalışma evrakıdır; oradaki bir iz en fazla "[BİLGİ] iz var ama TEYİT SAYILMAZ (model çıktısı)" şerhi alır, statüyü asla TEYİTLİ yapmaz. Teyitsiz atıf → **exit 1**. **AYRIŞTIRILAMAYAN ATIF DA BLOKTUR (v0.5.14/B-2, fail-closed):** merci anılan bir satırda karar iması (`2020/1111`, "sayılı/tarihli/günlü kararı") var ama künye çıkarılamıyorsa sonuç "atıf yok — kapı AÇIK" DEĞİL, "mekanik teyit YAPILAMADI — kapı KAPALI"dır; ayrıca desen kümesi `E:2020/1111`, küçük harf `esas/karar`, AYM `B. No: 2019/12345` (bireysel başvuruda E./K. YOKTUR — 6216 s. K. m.45-49) ve AİHM `Application no.` biçimlerini de görür. Dilekçenin KENDİ `DOSYA NO`/`ESAS NO`/`MERCİ` satırı bu kapıdan muaftır (B1). **K4 — «EKSİK KÜNYE» sınıfı (v0.5.16, Hamle 3; Kararlar #3/#4):** tanıyıcı üç deseni daha görür — (a) **tarih-only** «Yargıtay 9. HD'nin 12.03.2019 tarihli kararı/ilamı» ve (b) **K-only/E-only** «…'nin 2019/4444 sayılı kararı», «K. 2021/2222 sayılı», «E. 2020/1111 sayılı» artık `kunye_ortak.ayristirilamayan_atiflar`'ın **EKSİK KÜNYE** sınıfıdır (`sinif` alanı; tarih-only BLOK = Karar #3) → `kunye_teyit`te **[TEYİTSİZ] + exit 1**, `ictihat_muhakeme_denetim` G2'de **BLOK** («esas+karar tamamlanmalı»); (c) **BİRLEŞİK** «2020/1111-2021/2222 sayılı», «2019/4444-5555 sayılı» ve «2020/1111 E., 2021/2222 K.» biçimleri TAM künye olarak esas+karar'a **ayrıştırılır** (Yargıtay birleşik biçimi, `bicim='birlesik'`); HGK/CGK/İBK esas numarası «2020/9-111» tek sayı olarak korunur (kurul mercisinde etiketsiz «YYYY/N-N» bölünmez → EKSİK KÜNYE). Yanlış-BLOK yasağı: tetik **merci anılan satırla** sınırlıdır ve «tarihli/günlü» ancak «karar/ilam» ile bitişikse imadır — «28.02.2018 tarihli Resmî Gazete», «05.12.2024 tarihli ihtarname» BLOK değildir; ayrıca (onarım 2026-09-07) tarih-only iması ancak **tarih merciye aitse** geçerlidir — merci tarihten önce anılmalı ve arada başka bir mahkeme iyeliği («yerel mahkemenin», «ilk derece mahkemesinin») olmamalı: «Yerel mahkemenin 05.06.2024 tarihli kararı Yargıtay'ın yerleşik içtihadına aykırıdır» BLOK değildir (yapısal ölçüt; script hangi kararın kastedildiğini yorumlamaz). **Etiketli birleşik** «E. 2020/1111-2021/2222 [K.] sayılı» tabanla aynı biçimde TAM künyeye ayrışır (hakem K4 regresyonu kapatıldı: `_YN` tire soneki kelime sınırıyla sınırlı); aynı-yıl «E. 2019/4444-5555» satırda kurul yok ve numaralı daire varsa E-K diye bölünür, dairesiz/kurulsuz satırda bütün kalır (E-only → EKSİK KÜNYE, fail-closed). (Script künyenin KÜTÜK/DÖKÜM izini garantiler; hükmün iddiayı gerçekten karşıladığı A-2/A-5 muhakemesi yine aşağıdaki listenin işidir — mekanik iz ≠ içerik doğruluğu.)
...
- `python scripts/ictihat_muhakeme_denetim.py <taslak.md> --kok . [--tip <tip>]` → **İçtihat Muhakeme Zinciri mekanik kapısı** (MODÜL 2): dilekçedeki HER içtihat künyesi için `_oa/cikti/*ictihat-muhakeme*.md` kaydı var mı + künye eşleşiyor mu (`kunye_ortak.kunye_normalize` — esas/karar VE, biliniyorsa, **DAİRE**; esas/karar no'ları her dairede yılda sıfırdan başladığından tek başına esas/karar eşleşmesi YETERSİZDİR, daire de eşleşmelidir; dilekçe atfı daire belirtmiyorsa VE aynı esas/karar no'suna sahip birden fazla FARKLI daireye ait kayıt varsa **fail-closed exit 1** — "hangi kaydın geçerli olduğu belirlenemiyor") + KAYNAK-İZİ `_oa/teyit/dokum` içinde gerçekten var ve künye orada dize olarak geçiyor mu + İLGİLİ-KISIM/DAVAYA-BAĞ/DAMGA dolu mu (yoksa **çıplak atıf → exit 1**); ayrıca DAMGA=ALEYHE → **exit 1** (anayasa m.6 teslim engeli), DAMGA=ALEYHE-AYIRT ile AYIRT-ETME boşsa **exit 1** (fail-closed), DAMGA yok/geçersiz → **exit 1** ("muhakeme edilmemiş"), DAMGA=NOTR: G2 düzeyinde uyarı, [G6] triyaj kapısında **BLOK** (exit 1, v0.5.8.5). Script içeriğin isabetini MUHAKEME ETMEZ — yalnız varlık+bağ+alan bütünlüğünü denetler; doğrulanmış içtihat atfı hiç yoksa "emsal içtihat yok — muhakeme zayıf" **uyarısı** basar (bloklamaz). **`--tip` (M3-2/R6):** bu G1 uyarısı yalnız "esaslı" dilekçe tiplerinde (dava/cevap/istinaf/temyiz/aym_bireysel) basılır; `yemin`/`idari-kanal` gibi hafif tiplerde [BİLGİ]'ye düşer (G2/G3 engelleri tip'ten ETKİLENMEZ — `--tip` yalnız G1 gürültüsünü azaltır, tek kaynak liste `ESASLI_OLMAYAN_TIPLER`). **YENİ-2 (backlog):** aynı esas/karar/daireye ait birden çok kayıt varsa VE DAMGA değerleri birbirinden farklıysa (ör. biri LEHE biri ALEYHE), "ÇELİŞEN DAMGA" **uyarısı** basılır (bloklamaz) — script temiz adayı bulup kullanabilir ama tutarsızlığı sessizce gizlemez. **v0.5.16 ekleri:** (i) **G2 EKSİK KÜNYE (K4):** tarih-only/K-only/E-only atıf muhakeme kaydıyla eşleşemeyeceğinden çıplak atıf gibi **BLOK** (`eksik_kunye_bloklari`; makine kaynakça bloğu B-18 maskesiyle görülmez). **Kapsam notu (bilinçli genişleme):** aynı fonksiyon yalnız EKSİK KÜNYE sınıfını değil, `kunye_ortak.ayristirilamayan_atiflar`'ın genel **AYRIŞTIRILAMAYAN** sınıfını da (merci + karar iması var, hiçbir künye kalıbı tanınmadı) G2'de **BLOK** eder — `kunye_teyit` B-2 ile simetri: künyesi çıkarılamayan atıf muhakeme kaydıyla da eşleştirilemez, kapı fail-closed kalır; iki script aynı satır için aynı hükmü verir (tek kaynak `kunye_ortak`). (ii) **[G5-AKIBET] (K5, Hamle 4):** D grubunun `oa_hafiza.py teyit --akibet` bayrağı muhakeme kaydına `**AKIBET:**` (enum `kesinlesti|kesinlesmedi|bozuldu|kaldirildi|geri_cevrildi`) + `**AKIBET-KAYNAK:**` satırlarını, kütük hücresine `AKIBET=<enum>` tokenını yazar; bu script yalnız **okur** (`akibet_denetimi`, kütük esas): DAMGA=LEHE + AKIBET∈{bozuldu,kaldirildi} + dilekçede atıf → **BLOK** («bozulmuş/kaldırılmış karar LEHE dayanak olamaz» — HMK m.371 bozma sebepleri, m.353/1-a kaldırma; Mevzuat MCP teyit 2026-09-06); LEHE + `kesinlesmedi` → **UYARI** («kesinleşmemiş — akıbet izlenmeli, kanun yolu açık»); ALEYHE + bozuldu → **UYARI** (cephanelik bilgisi); AKIBET yoksa kapı ateşlemez (geriye uyum); enum dışı değer görünür uyarı, BLOK değil. Kayıt satırı VE kütük tokenı aynı normalizasyondan geçer (`_akibet_normalize`: Türkçe harf katlama, boşluk/tire → alt çizgi — «kesinleşti», «Geri-Çevrildi» enum'a indirgenir; onarım 2026-09-07). Rapor bölümü `[G5-AKIBET]`, exit sözleşmesi G5 ile aynı. (iii) **G11 (Hamle 11):** `ortusme_zenginligi_uyarisi` artık «(1)…; (2)…; (3)…» parantezli iç sıralamayı ve «;» ayraçlı parçaları da sayar — tek paragrafa yazılmış üç somut örtüşme noktası «yüzeysel» uyarısı almaz; tek cümle yine alır (advisory, bloklamaz). **(iv) v0.5.16.1 — KÜTÜK OKUYUCU SÜTUN TOLERANSI (saha yan-bulgusu):** `kunye_ortak` kütük okuyucuları (`kutukten_son_damga` / `kutukten_son_akibet` / `kutukte_esas_karar_satiri_var_mi` / `kutuk_gercek_veri_var_mi` / `kutukte_damgali_dayanak_satiri_var_mi`; bu scriptin `_kutuk_kunye_bilgisi` / `farkindalik_denetimi` ve `kunye_teyit.kutuk_ham_baglari` de aynı `kutuk_veri_satirlari` kapısından geçer) satırı artık SÜTUN SAYISINA göre elemez: gerçek dava kütüklerinde satırlar 17 hücreliydi (genişletilmiş düzen; `oa_hafiza.py teyit` 5 sütun/7 hücre yazar) ve eski `len != 7 → BOZUK` katılığı 26/33 taslakta her künyeyi «damgasız» gösterip [F] hafif kipi ve kütük teyidini kör bırakıyordu. Esas+karar eşleşmesi ve `DAMGA=` / `AKIBET=` / `DOKUM-SINIFI=` / `DUYULMUS=` tokenları hücre konumundan bağımsız, satırın TAMAMINDA aranır; daire künyeyi taşıyan hücreden okunur (daire-kör değil); yalnız `|` ile başlamayan / başlık / ayraç satırları sessiz atlanır; «BOZUK» uyarısı yalnız tek hücre bile kuramayan (kapanmamış `|`) satıra; birden çok `DAMGA=` → sonuncusu. Yazıcı simetrisi: `oa_hafiza.py teyit` sorgu hücresini de DAMGA=/künye-izi kaçışından geçirir (satır-geneli aramanın açtığı `--sorgu` enjeksiyon kapısı kapalı). Test: `tests/test_v0516_yan_bulgular.py`.
...
İZLİDİR, kaybolmaz) ATOMİK yazar. **Makbuz şemasının TEK YETKİLİ tanımı
`references/cikti-semasi.md` §6'dır** (v0.5.14/B-33: bu paragraf 11 alan
sayarken üretici 17 alan yazıyordu; alan listesi artık tek yerde yaşar ve
`tests/test_v0514_teslim.py` sürüklenmeyi kilitler — burada TEKRARLANMAZ).
**Makbuz garantisi ARGÜMANLAR AYRIŞTIKTAN sonra başlar** (v0.5.14/B-24):
argparse hatası (argümansız çağrı, geçersiz `--taraf`) artık `_oa/defter/`
AÇMAZ ve makbuz YAZMAZ — bir yazım hatası dosya sistemini değiştirmemeli ve
sahte bir dava kökü işareti bırakmamalıdır. Argümanlar ayrıştıktan sonraki
her başarısız yol RED makbuzu düşürmeye DEVAM EDER (sözleşme korunur).
Kapı başına durum ENUM'u {OK,BLOK,ATLA,BILGI}
...
## UDF TESLİM KAPILARI + MAKBUZ GARANTİSİ (v0.5.8.4 — saha devşirmesi)
...
- **PROV-TAZELİK:** mühürdeki sha güncel dosyayla uyuşmuyorsa (bayat mühür) teslim RED — bayat mühürle teslim YOK. **v0.5.14/B-13:** `artifact_sha256` alanı OLMAYAN mühür artık ayrı `shasiz` sınıfıdır ve RED'dir (eskiden sessizce "bayat" etiketiyle yutuluyordu; mührün İÇİNİ silmek, dosyayı hiç mühürlememekten TEMİZ görünüyordu). **v0.5.14/B-14:** filo-tazelik taraması artık `.udf` + `.pdf` + `.docx` kapsar (`TESLIM_SINIFI_UZANTILAR` tek sabiti) — sha karşılaştırması uzantıdan bağımsızdır; makbuz sonrası değişiklik penceresi `.docx` için de kapandı. Mühürsüz `.pdf`/`.docx` **advisory**dir (40-UYAP'a kopyalanan PDF/DOCX yoldaşları tanım gereği mühürsüz doğar; onları RED saymak her yeşil teslimi bir sonraki koşuda kırardı) — mühürsüz `.udf` kuralı v0.5.10'daki gibi kalır.
...
- **ŞEKİL:** pageFormat 4 kenar 42.52 pt (Yönetmelik 2646 m.8 — üst/sol/sağ 1,5 cm; alt kenar maddede yok, aile standardı olarak simetrik) değilse `udf_yaz`'ın kenar yaması OTOMATİK uygulanır ve mühür sha'sı GÜNCELLENİR; düzeltilemezse RED. `LineSpacing="0.50"` (1,5 satır) yaygınlığı ve 11pt link imzası yalnız İSTİŞARİ satırdır (kapı kapatmaz).
...
- **MAKBUZ GARANTİSİ:** zincir try/finally sarmalayıcıdadır — **argümanlar AYRIŞTIKTAN sonraki** her başarısız yol (taslak yok, kapı kapandı, beklenmeyen çökme) bir RED makbuzu düşürür (zaman + sebep + argv). **v0.5.14/B-24 sınırı:** argparse hatası (argümansız çağrı, geçersiz `--taraf`) ve var olmayan `--kok` makbuz YAZDIRMAZ ve `_oa/` AÇMAZ — kullanıcı hatası dosya sistemini değiştirmemeli, sahte dava kökü işareti bırakmamalıdır.
...
**`kaynakca_uret.py` — DÜRÜST BEYAN KURALI (v0.5.14/B-3, P0):** taslağın sonuna işlenen `## İÇTİHAT KAYNAKÇASI` bloğu, künyelerin TEYİT DURUMUNA BAKMADAN "tamamı tam metinleriyle okunup kütüğe damgalanmıştır" diyordu — muhakeme kaydı HİÇ olmayan uydurma bir künye için bile. Bu, avukatın imzasını taşıyacak belgeye mekanik olarak YALAN yazmaktır. Yeni kural: **teyit kütüğünde karşılığı olmayan tek bir künye varsa "tam metniyle okundu" cümlesi HİÇ yazılmaz**; o künye `⚠ TEYİT EDİLMEDİ` satırı alır ve makbuza `teyitsiz` sayısı düşer. Ayrıca künye çıkarımı artık `kunye_ortak`tan gelir (tek kaynak) — kapı neyi görüyorsa kaynakça da onu görür.

**İDEMPOTANS (v0.5.14/B-18):** makine üretimi kaynakça bloğu (işaretler arası, üretecin kendi başlık/önsöz/madde satırları) denetim girdisi SAYILMAZ — `kunye_ortak.makine_blogu_maskele` ile maskelenir. Eskiden bloğun yazdığı `⚠` işareti ikinci koşuda dilekçe denetiminin [C] kapısını kapatıyor ve aynı komut aynı dosyada `rc=0 → rc=1 → rc=1` veriyordu; kapı deterministik olmak zorundadır. İşaretler arasına ELLE yazılmış düz metin maskelenmez — kapı, metne yorum işareti yazarak atlatılamaz.

**`kaynak_blogu.py` (YENİ):** her `_oa/cikti` ürününün ilk satırına konacak `<!-- kaynaklar: yol@sha8 · ... -->` bloğunu `python scripts/kaynak_blogu.py --girdiler <yol...> [--besledigi X] [--uretim Y]` üretir — sha'yı model değil script hesaplar; `tazelik_denetim.py` bu blokla çalışır (@sha8'siz blok tazelik denetimini fiilen işlevsiz bırakır — saha bulgusu, v0.5.8.4).
...
**Üslup (P1-11 ek kural — ÜSLUP BAĞLAMASI, kanun yolu playbook'u):** [ ] üslup playbook'a uygun mu? — `oa-dilekce/references/kanun-yolu-mimari-playbook.md` (B1-B7) üslubuyla (tez-omurgalı, akıcı, bütünsel bağlantılı) örülmüş mü; GÖRÜNMEZ İSKELET (İDDİA→NORM→İÇTİHAT→ÖRTÜŞME→SONUÇ) yüzeye ETİKET olarak sızmamış mı ([H] advisory), karşı-taraf-kusuru bağlamında GİDERİLMESİNE yönelik onarma-talebi kurulmamış mı ([I] advisory) — `dilekce_denetim.py`'nin [H]/[I] uyarıları gözden geçirildi mi?
**İLK SAYFA + UZUNLUK testi (P2-7 / A-24, v0.5.16 — lens, kapı değil):** [ ] **İlk sayfa tek başına tezi taşıyor mu?** Dosyayı ilk kez açan hâkim yalnız ilk sayfayı (başlık bloğu + ilk paragraf + varsa özet) okuduğunda davanın NE olduğunu, NE istendiğini ve NEDEN haklı olduğumuzu anlıyor mu; tez ilk sayfada değil 6. sayfada başlıyorsa yeniden tertip. [ ] **Bölüm sayısı ve boyut GÖRÜNÜR raporlanır:** hakem, taslağın `##` bölüm sayısını ve dosya boyutunu (KB) rapora YAZAR (ör. «11 bölüm · 38 KB») — **eşik SAYI DEĞİL, BANT**: sınır bir rakam olarak yazılmaz; hakem dosyanın karmaşıklığına (taraf sayısı, talep sayısı, delil hacmi) göre «kısa / makul / uzun / şişkin» bandını ve gerekçesini yazar (basit bir cevap dilekçesinin 40 KB'ı «şişkin», çok-taraflı bir istinafın 40 KB'ı «makul» olabilir). «Uzun» ve «şişkin» bant için: hangi bölümlerin ek/cephanelik olarak dışarı alınabileceği önerilir; bu bir teslim engeli değil, tertip önerisidir.
...
**Mekanik kapıların hepsi yeşilken bile içerik yanlış olabilir.** Bir saha
...
çelişikti** — karşı tarafın rakamı zaten `(toplam − kalem)` idi, yani kalem o
hesabın DIŞINDAYDI; taslak ise "toplamın içinde, düşülmeli" diyordu. Dilekçe
...
**İKİNCİ PAS — HÂKİM LENSİ (A-21 / P1-5, v0.5.16; lens, kapı değil):** birinci
pas «çürütmeye çalış» brifiyle karşı vekilin gözüdür; ikinci pas aynı denetçinin
**dosyayı on dakikada okuyan hâkimin** gözüdür. Brif: *"Bu dosyaya on dakikan
var; ne istendiğini, neden, hangi delille anladın mı? Anlamadıysan nerede
kayboldun?"* Bu pas puan vermez, kapı kapatmaz; bulgularını "Açık uçlar ve
riskler" altında **tertip önerisi** olarak yazar:
- [ ] **On dakika testi:** ilk sayfa + başlıklar + netice-i talep okununca tez
      tek cümleyle söylenebiliyor mu? Söylenemiyorsa tez gömülüdür — öne
      çıkarılır (bkz. B listesi «İLK SAYFA + UZUNLUK testi»).
- [ ] **Talep bloğu tek başına yeterli mi:** netice-i talep, gerekçeyi
      okumamış bir hâkim için kendi başına anlaşılır ve HMK m.119/1-ğ
      anlamında «açık» mı (Mevzuat MCP teyit 2026-09-06)? Her talep kalemi
      hükme yazılabilir kesinlikte mi (miktar, faiz başlangıcı, taraf)?
- [ ] **Gerekçe alışkanlığı:** bu mahkeme/daire gerekçesini nasıl kurar —
      kısa özet + norm + "kabul edilmiştir" mi, yoksa delil delil tartışan
      uzun gerekçe mi? Dilekçe hâkimin kopyalayıp gerekçeye alabileceği
      (özetlenebilir, kaynaklı, tek paragraflık) blokları veriyor mu?
- [ ] **Daire eğilimi:** temyiz/istinaf mercii belli ise o dairenin bilinen
      eğilimi (usul titizliği, bilirkişiye bakış, faiz/tazminat yaklaşımı)
      kütükte teyitli kararlarla görülmüş mü — eğilim kütüğe dayanmadan
      «bilinir» diye yazılmaz (m.4/m.5: hafızadan eğilim uydurulmaz).
- [ ] **İş yükü:** yoğun bir mahkeme için dilekçe okuma maliyeti yüksek mi —
      tekrar, gereksiz alıntı, dilekçeye gömülü cephanelik var mı? Hâkimin
      atlayabileceği her sayfa tezi zayıflatır; kısaltma önerisi yazılır.
Lens, kapı DEĞİLDİR: bu pas hiçbir mekanik exit'i etkilemez; bulgusu avukat
kararına sunulan tertip/uzunluk önerisidir.
