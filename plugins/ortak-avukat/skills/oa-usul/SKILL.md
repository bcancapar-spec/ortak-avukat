...
1. **Norm (Mevzuat MCP — tam batarya):** `mevzuat_ara` önce **tipsiz** (yalnız KANUN/KHK/CB_KARARNAME/TUZUK gelir — yanıttaki `aranmayan_turler`e bak), sonra `mevzuat_tur_list` ile 4'erli tür süpürmesi (YONETMELIK/CB_YONETMELIK/KKY/UY · TEBLIGLER/CB_KARAR/CB_GENELGE/MULGA); madde içinde `mevzuat_icinde_ara`. "Genel kanunda yok" demek "mevzuatta yok" demek DEĞİLDİR — özel usul, genel usulü değiştirebilir (lex specialis).
...
4. **KAPILARI KAPAT (çökertme adımı) — İKİ KATMAN AYRIDIR (ANAYASA m.6):** karşı tarafın başvurabileceği kurtuluş kapılarını ÖNGÖR — eski hâle getirme mi diyecek (mazeret unsuru var mı?), tebliğin usulsüzlüğünü mü ileri sürecek (7201 m.32 öğrenme tarihi?), tamamlanabilir eksiklik mi diyecek? Her öngörülen kapı için **kapatma argümanını** (unsur eksikliği, süre, içtihat) hazırla. Sonra iki katmanı ASLA karıştırma:
   - **(a) DAHİLİ HAZIRLIK — daima EKSİKSİZ:** öngörülen her kapı ve kapatma argümanı **iç dosyaya** yazılır (`usul_matris.py` `kapi_kapatma` kaydı + `_oa/cikti/06-antitez-cephanelik.md`). Motorun [G2b] kapısı **bu katmanı** denetler; hiçbir kapı öngörülmeden karşı tarafın kaçırması "kapatılmış" sayılmaz.
...
6. **TAMAMLANABİLİR × TAMAMLANAMAZ KUSUR — "NE ZAMAN İLERİ SÜR" (v0.5.16, P2-3/A-8):** Karşı tarafın usul kusuru iki sınıftır ve ikisinin zamanlaması AYNI DEĞİLDİR:
   - **TAMAMLANAMAZ kusur (süre):** kaçırılmış süre telafisizdir; sonucu kesindir → **"derhâl"** kuralı (anayasa m.2: *"gizli tutulmaz, net dille çalışmaya eklenip derhâl ileri sürülür"*) **bu sınıf içindir.**
   - **TAMAMLANABİLİR kusur (harç, vekâletname, dilekçe eksiği, giderilebilir dava şartı):** mahkeme eksikliği fark edince **kesin süre verir** ve karşı taraf **tamamlar** — HMK m.119/2 ve m.115/2 çıpaları: m.115/2 (*"dava şartı noksanlığının giderilmesi mümkün ise bunun tamamlanması için kesin süre verir"*), m.119/2 (dilekçenin (a)(d)(e)(f)(g) dışı eksiklerinde **bir haftalık kesin süre**; tamamlanmazsa dava açılmamış sayılır), m.77 (vekâletnamesiz işlemde kesin süre; verilmezse işlem yapılmamış sayılır) — **Mevzuat MCP teyit 2026-09-06**; harç ikmali 492 m.30-32 (çıpa — teyit edilmedi, cetvel §4). Bu kusuru **erken ileri sürmek karşı tarafa tamamlama fırsatı verir**; geç ileri sürmek (ör. kesin sürenin kendiliğinden kaçmasını izlemek, hükme yakın dile getirmek) veya hiç ileri sürmeyip mahkemenin re'sen gözetmesini beklemek meşru taktiklerdir. **"Ne zaman ileri sür" bu yüzden AVUKAT KARARIDIR** — script karar vermez, kararın yazılıp yazılmadığına bakar.
   - **Anayasa m.2 ile çelişki YOKTUR — yorumdur:** m.2'deki "derhâl", *telafisi olmayan* usul hatası (süre) için yazılmıştır; tamamlanabilir kusurda "derhâl" ileri sürmek karşı tarafı onarır ve m.6'nın taktik yüzüne (rakibin dosyasını onarmasına yardım etmeme — aşağıdaki KUSUR→SONUÇ→TALEP asimetrisi) ters düşer. İki hüküm birlikte okunur: *tamamlanamazda derhâl, tamamlanabilirde avukat kararı.*
   - **Motor kaydı:** `usul_matris.py` işlem kaydında opsiyonel `tamamlanabilir: true|false` + `zamanlama: simdi|sonra|avukat_karari` (advisory — şema `references/usul-cetveli.md` §4.a; eksikse «tamamlanabilir kusurda zamanlama kararı yok» satırı, BOŞLUK değil; exit sözleşmesi korunur).
...
## GÖREV DÖNGÜSÜ — MERCİ TAYİNİ (Hamle 12, v0.5.16 — kamu bilgisi, m.7 uyumlu)

**Örüntü (saha dersi, soyutlanmış):** İlçe asliye hukuk mahkemesinin **asliye ticaret sıfatıyla** baktığı bir dosya, HSK Genel Kurulu'nun **07/07/2021 tarihli ve 608 sayılı** kararıyla (asliye ticaret mahkemelerinin yargı çevrelerinin il mülki sınırı olarak yeniden belirlenmesi; **01/09/2021**'den itibaren uygulanır) ihtisas değişikliğine uğrar. İlçe mahkemesi HSK 608'e dayanarak dosyayı il merkezindeki asliye ticaret mahkemesine **devreder**; il mahkemesi **aynı HSK 608'e** dayanarak (karar 01/09/2021'den sonra açılan davalara uygulanır; derdest dosyaya devir olmaz) dosyayı **geri gönderir**. İki mahkeme de aynı karara dayanır → **devir ↔ görevsizlik ping-pong'u**; dosya aylarca esasa giremez (anayasa m.2: usulde kaybedilen zaman esas hakkını eritir).

**Döngü kırıcı = DIŞ HAKEM:** ping-pong'un tarafı olma. Doğrudan **MERCİ TAYİNİ (yargı yeri belirlenmesi)** talebi:
- **HMK m.21/1-c:** *"İki mahkeme de görevsizlik kararı verir ve bu kararlar kanun yoluna başvurulmaksızın kesinleşirse"* yargı yeri belirlenmesi yoluna başvurulur; **m.22/2:** görevli mahkeme *"ilgisine göre bölge adliye mahkemesince veya Yargıtayca belirlenir"*; **m.23/2:** belirleme kararı *"davaya ondan sonra bakacak mahkemeyi bağlar"* — **Mevzuat MCP teyit 2026-09-06.** İnceleme dosya üzerinden yapılabilir (m.23/1).
- **BAM'ın tutumu (Yargı Pro teyit 2026-09-06 — tam metin okundu):** HSK 608'e dayalı karar *"gerçekte görevsizlik kararı değil, teknik anlamda bir **gönderme kararı**dır"*, nihai hüküm niteliğinde değildir ve **istinaf yolu kapalıdır**; istinaf istemi HMK m.341/346 gereğince **usulden reddedilir** ve dosya *"yargı yerinin belirlenmesi (merci tayini) prosedürünün uygulanması için"* ilk derece mahkemesine iade edilir — **Antalya BAM 4. HD, E. 2023/126, K. 2023/67, 24.01.2023 (kesin)**; aynı gün aynı daire E. 2023/128 K. 2023/76 aynı yönde. Karşı örnek de öğreticidir: Gaziantep BAM 4. HD, E. 2023/632 K. 2023/795, 15.06.2023 — ikinci mahkemenin kararı *gerçek* görevsizlik (ticari dava değil → asliye hukuk) olduğundan "karşılıklı görevsizlik" şartı henüz oluşmamış sayılıp dosya geri çevrilmiştir. **Ders:** merci tayini isterken iki kararın da aynı uyuşmazlık hakkında ve karşılıklı olduğunu, kesinleştiğini belgele (m.21/1-c "kanun yoluna başvurulmaksızın kesinleşme" şartı — gönderme kararına istinaf zaten kapalı olduğundan kesinleşme kendiliğinden oluşur; BAM bunu söyler).
- **Ne yapılır:** gönderme kararını istinaf etmek yerine (harç + süre kaybı + usulden ret) ilk derece mahkemesinden dosyanın **merci tayini için BAM'a gönderilmesi** talep edilir; talep dilekçesinde iki kararın tarih/kesinleşme zinciri ve HMK m.21/1-c + m.22/2 dayanağı yazılır. Talep reddedilirse m.22/1'deki "engel" hâli de aynı kapıdır.

**Örneklem ilkesi (anayasa m.3):** bu örüntü **tüm görev/yetki döngülerine kıyasen** uygulanır — görev↔görev (m.21/1-c), kesin yetki↔kesin yetki (m.21/1-ç), yargı çevresi tereddüdü (m.21/1-b), yargı yolu uyuşmazlığı (Uyuşmazlık Mahkemesi — ayrı rejim, teyit) dahil. Kural tek cümledir: **iki merci birbirine gönderiyorsa üçüncü (dış) hakemi devreye sok; döngünün tarafı olma** (dış hakem = döngü kırıcı). Künye kullanım anında `oa-ictihat` ile yeniden teyit edilir; HSK karar sayısı kamu bilgisidir, dosya/kişi verisi değildir.
