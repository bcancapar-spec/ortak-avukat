...
def test_sure_adaylari_liste_ve_json(tmp_path):
    ...
    oneri_satirlari = [s for s in cikti.splitlines()
                       if s.startswith("  - ") and "hesapla_sure.py --kural" in s]
    assert len(oneri_satirlari) == 3, oneri_satirlari
    assert all("--flagsiz" in s for s in oneri_satirlari), oneri_satirlari
    # sure-flag YAZILMADI
