# v0.5.16/G — Yargı Pro teyit izi (hakem)
...
## G'ye özgü hüküm
- G diff'inde hukuki norm veya içtihat atfı YOK (yapısal kilitler + hata tetikli damıtma belgesi); MCP teyidi gerektiren iddia bulunmadı.
- KANONİK↔doktrin kilidi (b) A grubunun enum tablosuyla uyumlu (A'da tablo mevcut).
- Küçük (sözleşme uyumsuzluğu): oa-usta/SKILL.md «--sebep <olgu|uslup|strateji|eksik|fazla>» yazıyor; B'nin `pipeline_kayit.py` `HUKUM_SEBEPLERI` = usul|olgu|hukuk|uslup|talep|ictihat|diger. G belgesi B'nin kapalı listesine göre düzeltilmeli (strateji/eksik/fazla script tarafından RET edilir).
